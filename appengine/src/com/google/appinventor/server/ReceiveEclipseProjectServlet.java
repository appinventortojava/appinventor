...
/**
 * Servlet for receiving build output files and build error information from a
 * Build Server.
 * 
 * <p>
 ...
 * safety they contain an encrypted user and project ID as part of their URL.
 * 
 * @author markf@google.com (Mark Friedman)
 */
public class ReceiveEclipseProjectServlet extends OdeServlet {

	// Logging support
	private static final Logger LOG = Logger
			.getLogger(ReceiveEclipseProjectServlet.class.getName());
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
			throws IOException {
		// URIs for receiveEclipseProject requests are structured as follows:
		// /<baseurl>/receiveEclipseProject/encryptedUserAndProjectId/fileId
		String uriComponents[] = req.getRequestURI().split("/", 6);

		// TODO(lizlooney,user) If the URI doesn't contain enough components,
		// the following lines
		// will throw an ArrayIndexOutOfBoundsException. We could deal with that
		...
		// Set the user in the OdeFilter, which is used everywhere as the
		// UserInfoProvider.
		odeFilter.setUserFromUserId(userId);
		try {
			String buildFileDirPath = uriComponents[5];
			...
			while (true) {
				...
				if(Integer.parseInt(uriComponents[4]) == 1)
					storageIo.addEclipseProjectOutputFilesToProject(userId,
						projectId, filePath);
				else
					storageIo.addJavaSourceOutputFilesToProject(userId,
							projectId, filePath);
